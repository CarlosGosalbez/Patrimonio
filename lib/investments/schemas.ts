...
const investmentTypeSchema = z.enum(['stock', 'etf', 'fund', 'crypto', 'deposit', 'bond', 'reit', 'other'])
...
function thresholdInputSchema(required = false) {
  const base = z
    ...
    .refine(
      (value) =>
        value === null || (typeof value === 'number' && Number.isFinite(value) && value >= 0.1 && value <= 50),
      {
