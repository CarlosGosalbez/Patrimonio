...
export async function updateInvestmentPosition({
  ...
  if ('next_dividend_date' in input) {
    updatePayload.next_dividend_date = (input.next_dividend_date as string | null | undefined) ?? null
  }
  ...
  if ('annual_dividend_per_share_input' in input && typeof input.annual_dividend_per_share_input === 'string') {
  ...
  await syncInvestmentFromMarketCache({
    investment: {
      currency: typeof updatePayload.currency === 'string' ? updatePayload.currency : existing.currency,
      id: existing.id,
...
export async function createInvestmentOperation({
  ...
  const withholdingCents = operationType === 'dividend' ? parseMoneyOrZero(input.withholding_input) : 0

  if ((operationType === 'sell' || operationType === 'transfer_out') && quantity > investment.quantity) {
  ...
  const priceCents =
    operationType === 'split'
      ? 0
      : parseCurrencyInput(String(input.price_input))
  const feeCents =
    operationType === 'split'
      ? 0
      : parseMoneyOrZero(input.fee_input)
