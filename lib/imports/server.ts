...
async function getImportContext({
  ...
  const [{ data: incomeCommitmentsData, error: incomeCommitmentsError }] = await Promise.all([
    supabase
      ...
      .select('id,name,account_id,amount_cents,is_income,next_due_date,tolerance_days,is_active,deleted_at')
      .eq('user_id', userId)
      .eq('is_income', true)
      .is('deleted_at', null),
  ])
  ...
  const incomeCommitments = ((incomeCommitmentsData ?? []) as Array<{
    account_id: string
    amount_cents: number
  ...
  return {
    ...
    incomeTransactions: (incomeTransactionsData ?? []) as Array<
      Pick<TransactionListRow, 'account_id' | 'amount_cents' | 'is_income' | 'recurring_id' | 'transaction_date'>
    >,
...
export async function confirmImport({
  ...
  try {
    ...
    for (const previewRow of analysis.rows.filter((row) => row.should_import && row.unexpected_charge)) {
...
export async function rollbackImportBatch({
  ...
  const rolledBackCount =
    Array.isArray(data) && data[0]?.rolled_back_count
      ? Number(data[0].rolled_back_count)
      : 0
