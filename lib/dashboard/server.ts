...
export async function getDashboardSummary({
  ...
  const [
    ...
    supabase
      ...
      .select('category_id,total_cents,transaction_count,category:categories(name,color)')
      .eq('user_id', userId)
      .eq('month', currentMonth)
  ...
  return {
    ...
    top_categories: (topCategoriesResult.data ?? []).map((row) => ({
      ...
      color: (row.category as { color?: string | null } | null)?.color ?? null,
      name: (row.category as { name?: string | null } | null)?.name ?? t('uncategorized'),
