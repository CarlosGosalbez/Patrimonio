...
export function buildMonthlyTrendCards(categories: CategoryMonthlySeriesInput[]) {
  return categories
    .map<MonthlyTrendCard | null>((category) => {
      ...
      const average =
        category.monthly_totals.reduce((sum, value) => sum + value, 0) / category.monthly_totals.length
      ...
      return {
        ...
        category_name: category.category_name,
        current_month_cents: category.current_month_cents,
        delta_percent: average > 0 ? roundOneDecimal(((category.current_month_cents - average) / average) * 100) : null,
      }
    })
