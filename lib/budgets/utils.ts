...
function toDateOnly(value: string | Date) {
  return value instanceof Date ? new Date(`${value.toISOString().slice(0, 10)}T00:00:00`) : new Date(`${value}T00:00:00`)
}
...
export function getBudgetWindow(
  ...
  if (period === 'annual') {
    const anchor = offset === 0 ? referenceDate : offset > 0 ? addYears(referenceDate, offset) : subYears(referenceDate, Math.abs(offset))
  ...
  const anchor = offset === 0 ? referenceDate : offset > 0 ? addMonths(referenceDate, offset) : subMonths(referenceDate, Math.abs(offset))
