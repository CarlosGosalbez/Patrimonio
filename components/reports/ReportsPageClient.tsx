...
function DeltaBadge({ pct, invert }: { pct: number | null; invert?: boolean }) {
    ...
    return (
        ...
        >
            {isGood ? <TrendingUp className="h-3 w-3" aria-hidden="true" /> : <TrendingDown className="h-3 w-3" aria-hidden="true" />}
            {pct > 0 ? '+' : ''}{pct.toFixed(1)}%
        </span>
    )
}

function MetricCard({ label, value, delta, invertDelta }: {
    label: string
    value: string
    delta?: number | null
    invertDelta?: boolean
...
function CategoryBar({ name, amountCents, pct }: { name: string; amountCents: number; pct: number }) {
    return (
        <div className="space-y-1">
            ...
            <div className="h-2 w-full rounded-full bg-muted" role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100}>
                <div
...
function MonthlyTab({ month, year }: { month: number; year: number }) {
    ...
    const { data: comparison, isLoading: cmpLoading } = usePeriodComparisonQuery('month', month, year)
    ...
    if (isLoading || cmpLoading) {
        return (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" aria-busy="true" aria-label={t('loading')}>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <MetricCard
                    label={t('netBalance')}
                    value={formatCurrency(data.net_balance_cents)}
                />
                <MetricCard
                    label={t('savingsRate')}
                    ...
                    <CardContent>
                        <ResponsiveContainer width="100%" height={200}>
                            ...
                            >
                                ...
                                <YAxis tickFormatter={(v) => formatCurrency(v, 'EUR')} tick={{ fontSize: 10 }} width={72} />
            ...
            <Card className="border-border/70 bg-card/90 shadow-sm">
                ...
                <CardContent className="divide-y divide-border">
                    ...
                    {data.top_expenses.length === 0 && (
                        <p className="py-4 text-sm text-center text-muted-foreground">{t('noData')}</p>
                    )}
                </CardContent>
            </Card>
        </div>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <MetricCard
                    label={t('netBalance')}
                    value={formatCurrency(data.total_net_cents)}
                />
                <MetricCard
                ...
                <CardContent>
                    <ResponsiveContainer width="100%" height={240}>
                        ...
                        >
                            ...
                            <Bar dataKey="expenses_cents" name={t('expenses')} fill="#dc2626" radius={[2, 2, 0, 0]} />
                        </BarChart>
                    </ResponsiveContainer>
                </CardContent>
            </Card>
    ...
    return (
        <div className="space-y-6">
            ...
            <Card className="border-border/70 bg-card/90 shadow-sm">
                ...
                <CardContent className="space-y-4">
                    <div className="grid gap-4 sm:grid-cols-3">
                        <MetricCard label={t('gains')} value={formatCurrency(data.capital_gains.total_gains_cents)} />
                        <MetricCard label={t('losses')} value={formatCurrency(data.capital_gains.total_losses_cents)} />
                        <MetricCard label={t('netResult')} value={formatCurrency(data.capital_gains.net_cents)} />
                    </div>

                    {data.capital_gains.operations.length > 0 && (
                        <div className="divide-y divide-border">
                            <div className="grid grid-cols-4 pb-2 text-xs text-muted-foreground font-medium uppercase tracking-wide">
                                <span>{t('ticker')}</span>
                            ...
                            {data.capital_gains.operations.map((op, i) => (
                                <div key={`${op.ticker}-${i}`} className="grid grid-cols-4 items-center py-2.5 text-sm">
                                    <div>
                                        <p className="font-medium">{op.ticker}</p>
                                        <p className="text-xs text-muted-foreground truncate">{op.investment_name}</p>
                                    </div>
                                    <p className="text-right text-muted-foreground">
                                        {new Date(op.operation_date).toLocaleDateString('es-ES')}
                                    </p>
                                    <p className="text-right">{formatCurrency(op.sell_price_cents)}</p>
                                    <p className={`text-right font-medium ${op.is_gain ? 'text-emerald-600' : 'text-rose-600'}`}>
                                        {op.gross_gain_cents >= 0 ? '+' : ''}{formatCurrency(op.gross_gain_cents)}
                                    </p>
                                </div>
                            ))}
                        </div>
                    ...
                    {data.capital_gains.operations.length === 0 && (
                        <p className="text-sm text-muted-foreground text-center py-4">{t('noSales')}</p>
                    )}
                </CardContent>
            </Card>
            ...
            <Card className="border-border/70 bg-card/90 shadow-sm">
                ...
                <CardContent className="space-y-4">
                    <div className="grid gap-4 sm:grid-cols-3">
                        <MetricCard label={t('grossDividends')} value={formatCurrency(data.dividends.gross_total_cents)} />
                        <MetricCard label={t('withholding')} value={formatCurrency(data.dividends.withheld_total_cents)} />
                        <MetricCard label={t('netDividends')} value={formatCurrency(data.dividends.net_total_cents)} />
                    </div>
                    ...
                    {data.dividends.rows.length > 0 && (
                        <div className="divide-y divide-border">
                            ...
                            {data.dividends.rows.map((div, i) => (
                                <div key={`${div.ticker}-${i}`} className="grid grid-cols-4 items-center py-2.5 text-sm">
                                    <div>
                                        <p className="font-medium">{div.ticker}</p>
                                        <p className="text-xs text-muted-foreground truncate">{div.investment_name}</p>
                                    </div>
                                    <p className="text-right text-muted-foreground">
                                        {new Date(div.operation_date).toLocaleDateString('es-ES')}
                                    </p>
                                    <p className="text-right">{formatCurrency(div.gross_amount_cents)}</p>
                                    <p className="text-right font-medium text-emerald-600">{formatCurrency(div.net_amount_cents)}</p>
                                </div>
                            ))}
                        </div>
                    )}
        ...
        <Card className="border-border/70 bg-card/90 shadow-sm">
            ...
            <CardContent className="space-y-4">
                ...
                <div className="flex items-start gap-2.5">
                    ...
                    />
                    <label htmlFor={confirmId} id={`${confirmId}-desc`} className="text-sm cursor-pointer">
                        {t('gdprConfirmLabel')}
                    </label>
                </div>
                <Button
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-2">
                    <label htmlFor={monthSelectId} className="text-sm font-medium whitespace-nowrap">
                        <CalendarDays className="inline h-4 w-4 mr-1 text-muted-foreground" aria-hidden="true" />
                        {t('month')}
                    </label>
                    <select
                        id={monthSelectId}
                ...
                <div className="flex items-center gap-2">
                    <label htmlFor={yearSelectId} className="text-sm font-medium">{t('year')}</label>
                    <select
                        id={yearSelectId}
                        value={selectedYear}
            ...
            <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
                <TabsList className="mb-4 flex flex-wrap h-auto gap-1">
                    <TabsTrigger value="monthly" className="min-h-[44px]">{t('tabMonthly')}</TabsTrigger>
                    <TabsTrigger value="annual" className="min-h-[44px]">{t('tabAnnual')}</TabsTrigger>
                    <TabsTrigger value="fiscal" className="min-h-[44px]">{t('tabFiscal')}</TabsTrigger>
                    <TabsTrigger value="gdpr" className="min-h-[44px]">{t('tabGdpr')}</TabsTrigger>
                </TabsList>
