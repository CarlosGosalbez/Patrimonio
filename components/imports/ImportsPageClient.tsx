...
export function ImportsPageClient() {
  ...
  const previewSummary = useMemo(
    () => previewMutation.data?.summary ?? null,
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("upload.detectedBank", { bank: t(`banks.${parsedFile.sourceBank}`) })} ·{" "}
                      {parsedFile.rawRows.length} {t("upload.rows")}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent className="text-2xl font-semibold">
              {previewSummary.duplicate_count + previewSummary.duplicate_in_file_count}
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
            ...
            <CardContent className="text-2xl font-semibold">
              {previewSummary.unexpected_charge_count}
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
            ...
            <CardContent className="text-2xl font-semibold">
              {previewSummary.expected_income_gap_count}
            </CardContent>
          </Card>
        </div>
      ) : null}
              ...
              <div key={batch.id} className="rounded-2xl border border-border/60 bg-muted/20 p-4">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {batch.account?.name} · {batch.source_format.toUpperCase()} ·{" "}
                      ...
                      <Button
                        ...
                        onClick={() =>
                          rollbackMutation
                            ...
                            .catch((error) =>
                              toast.error(
                                error instanceof Error ? error.message : t("toasts.rollbackError"),
                              ),
                            )
                        }
                      >
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted-foreground">
                  ...
                  <span>
                    {t("history.unexpectedCharges", { count: batch.unexpected_charge_count })}
                  </span>
                  <span>
                    {t("history.expectedIncomeGaps", { count: batch.expected_income_gap_count })}
                  </span>
                  <span>
                    {t("history.range", {
                      end: batch.source_range_end ?? "—",
                      start: batch.source_range_start ?? "—",
                    })}
                  </span>
                </div>
              </div>
