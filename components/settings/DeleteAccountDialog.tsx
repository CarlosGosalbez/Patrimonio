...
export function DeleteAccountDialog({ open, onOpenChange }: DeleteAccountDialogProps) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : t("deleteAccount.error"));
    } finally {
      setIsPending(false);
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="pt-2">{t("deleteAccount.warning")}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <div>
            <Label htmlFor={confirmId}>{t("deleteAccount.confirmLabel")}</Label>
            <p className="mb-2 text-xs text-muted-foreground">{t("deleteAccount.confirmHint")}</p>
            <Input
              id={confirmId}
