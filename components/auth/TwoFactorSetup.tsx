...
export function TwoFactorSetup({ enabled, factorId, onChange }: TwoFactorSetupProps) {
    ...
    const verifyAndActivate = async () => {
        ...
        if (challengeError) { setError(t('verifyError')); setLoading(false); return }
        ...
        if (verifyError) { setError(t('verifyError')); setLoading(false); return }

        const res = await fetch('/api/auth/recovery-codes', { method: 'POST' })
        const json = await res.json()
        setLoading(false)
        if (!res.ok) { toast.error(t('noRecoveryWarning')); setStep('done'); onChange?.(); return }
    ...
    const handleUnenroll = async () => {
        ...
        if (error) { toast.error(t('unenrollError')); return }
    ...
    if (step === 'recovery') {
        return (
            <RecoveryCodes
                codes={recoveryCodes}
                onDone={() => { setStep('done'); setRecoveryCodes([]) }}
            />
        )
    }
        ...
        return (
            <div className="space-y-4">
                <div className="flex items-center gap-2">
                    <Badge variant="outline" className="gap-1 text-green-600 border-green-200 bg-green-50 dark:bg-green-950/20">
                ...
                <Dialog open={showUnenroll} onOpenChange={setShowUnenroll}>
                    <DialogContent>
                        ...
                        <DialogFooter className="gap-2">
                            <Button variant="outline" onClick={() => setShowUnenroll(false)}>{tCommon('cancel')}</Button>
        ...
        return (
            <div className="space-y-5">
                ...
                <div className="space-y-1">
                    <Label className="text-xs text-muted-foreground uppercase tracking-wide">{t('orEnterManually')}</Label>
                    <div className="flex items-center gap-2">
                        <code className="flex-1 px-3 py-2 rounded-md bg-muted text-sm font-mono break-all">{secret}</code>
                ...
                <Button className="w-full" onClick={() => setStep('verify')}>{t('scannedIt')}</Button>
            </div>
        )
    }
        ...
        return (
            <div className="space-y-4">
                ...
                <div className="flex gap-2">
                    ...
                    >
                        {loading
                            ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" aria-hidden="true" /> {t('verifying')}</>
                            : t('activateConfirm')}
                    </Button>
                </div>
            </div>
