      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  setState((current) => ({ ...current, due_date: event.target.value }))
                }
                required
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                id="alert-amount"
                inputMode="decimal"
                value={state.expected_amount_input}
                onChange={(event) =>
                  setState((current) => ({ ...current, expected_amount_input: event.target.value }))
                }
              />
            </div>
            <div className="space-y-2">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input
                id="alert-advance"
                inputMode="numeric"
                value={String(state.advance_notice_days)}
                onChange={(event) =>
                  setState((current) => ({
                    ...current,
                    advance_notice_days: Number(event.target.value || 0),
                  }))
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                id="alert-snooze"
                type="date"
                value={state.dismissed_until}
                onChange={(event) =>
                  setState((current) => ({ ...current, dismissed_until: event.target.value }))
                }
              />
            </div>
          </div>
          ...
          <label className="flex min-h-[44px] items-center gap-2 rounded-2xl border border-border/60 px-4 py-3 text-sm">
            <input
              checked={state.auto_deactivate}
              type="checkbox"
              onChange={(event) =>
                setState((current) => ({ ...current, auto_deactivate: event.target.checked }))
              }
            />
